...
class SituatedQATemp(BaseDataset):
    def __init__(self, 
                 name:str, 
                 data_path:str,
                 seed:int=1234) -> None:
    ...
    def _load_dataset(self, filename:str='temp.jsonl') -> List[Dict]:
    ...
    def get_dataset(self) -> Dataset:
        ...
        ## load dataset from file
        train_data = self._load_dataset(filename='temp_train.jsonl')
        dev_data = self._load_dataset(filename='temp_dev.jsonl')
        test_data = self._load_dataset(filename='temp.jsonl')
