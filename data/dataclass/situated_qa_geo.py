...
class SituatedQAGeo(BaseDataset):
    def __init__(self, 
                 name:str, 
                 data_path:str,
                 seed:int=1234) -> None:
    ...
    def _load_dataset(self, filename:str='geo.jsonl') -> List[Dict]:
    ...
    def get_dataset(self) -> Dataset:
        ...
        ## load dataset from file
        train_data = self._load_dataset(filename='geo_train.jsonl')
        dev_data = self._load_dataset(filename='geo_dev.jsonl')
        test_data = self._load_dataset(filename='geo.jsonl')
